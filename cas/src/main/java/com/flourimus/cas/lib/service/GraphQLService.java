...
public class GraphQLService {
    ...
    /**
     * Sends a GraphQL query to the specified URL and returns the response as an object of type {@code classType}.
     *
     ...
     */
    public <T> T callGraphQLApi(final String targetUrl, final String query, final Map<String, Object> variables, final Class<T> classType) {
     ...
     */
    private <T> ResponseEntity<GraphqlResponse<T>> sendGraphQLRequest(String targetUrl, HttpEntity<GraphQLRequest> entity, Class<T> classType) {
    ...
    /**
     * Parses the GraphQL response and returns the response object of type {@code classType}.
     * 
     * @param responseEntity the ResponseEntity containing the GraphQL response
     * @param classType the class type of the response object
     * @return the response object of type {@code classType}
     * @throws ApiException if the GraphQL response body is missing or if errors are present in the response
     */
    private <T> T parseResponse(ResponseEntity<GraphqlResponse<T>> responseEntity, Class<T> classType) {
        ...
        if (body == null) {
            throw new ApiException("GraphQL response body is missing", CasErrorCodes.NO_RESPONSE_BODY, HttpStatus.SC_METHOD_FAILURE);
        }
